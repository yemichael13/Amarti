      ...
      <PageMotion>
        <div className="min-h-screen bg-gray-50 pt-30 pb-16">
          <Reveal className="px-6 md:px-20 mb-12">
            <h1 className="text-4xl md:text-6xl font-bold text-center text-green-800 mb-4">News</h1>
            <p className="text-center text-gray-600 max-w-2xl mx-auto">
              Updates and stories from Aamaartii.
            </p>
          </Reveal>

          {loading && (
            <div className="text-center text-gray-600 text-xl py-20">Loading…</div>
          )}
          ...
          {!loading && !error && posts.length === 0 && (
            <div className="text-center text-gray-600 text-xl py-20">No news yet. Check back soon.</div>
          )}
                ...
                {posts.map((post) => {
                  ...
                  return (
                    <Stagger.Item key={post.id}>
                      <Link
                        to={`/news/${post.slug}`}
                        className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 block h-full transform hover:-translate-y-2"
                      >
                        {imageUrl && (
